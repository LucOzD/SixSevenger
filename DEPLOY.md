...
compared with a logged-in user's weighted category-centroid vector. The most
relevant currently active campaign is placed after every 20 organic feed posts;
minimum similarity and frequency settings guide preference and rotation without
leaving the ad slot empty when inventory exists. Guests receive an untracked,
non-personalized active fallback.
