...
function renderNotifications(items) {
  ...
  items.forEach(item => {
    ...
    dismissBtn.addEventListener('click', async (evt) => {
      evt.stopPropagation();
      await fetch(`/notifications/${item.id}/dismiss`, { method: 'POST' });
      row.remove();
      const badge = document.getElementById('notificationBadge');
      if (badge) {
...
function navigateNotification(item) {
  ...
  if (item.type === 'comment' || item.type === 'like') {
    if (payload.postId) {
      const target = `/view-post/${payload.postId}` + (payload.commentId ? `?highlightCommentId=${payload.commentId}` : '');
  ...
  if (item.type === 'follow_request' || item.type === 'follow_accept') {
    if (payload.fromUserId) {
      window.location.href = `/user?id=${payload.fromUserId}`;
      return;
    }
  }
...
function createPostCard(post) {
  ...
  const userLink = `<a href="/user?id=${post.userId}">${post.username || 'Guest'}</a>`;
  ...
  commentToggle.addEventListener('click', async () => {
    ...
    if (expanded) {
      ...
      const comments = await fetch(`/post/${post.id}/comments`).then(r => r.json());
      commentCountEl.textContent = comments.length;
      ...
      if (deleteBtn) {
        deleteBtn.addEventListener('click', async () => {
          if (confirm('Are you sure you want to delete this post?')) {
            const res = await fetch(`/post/${post.id}/delete`, { method: 'POST' });
            const data = await res.json();
            if (data.success) {
              loadMyPosts(); // Refresh
            } else {
