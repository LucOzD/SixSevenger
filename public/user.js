    ...
    if (data.incomingRequestId) {
      if (acceptFollowButton) {
        ...
        acceptFollowButton.addEventListener('click', async () => {
          const response = await fetch(`/follow-request/${data.incomingRequestId}/accept`, { method: 'POST' });
      ...
      if (data.isFollowing) {
        ...
        followButton.addEventListener('click', async () => {
          const response = await fetch(`/unfollow/${id}`, { method: 'POST' });
          const result = await response.json();
          if (result.success) {
        ...
        followButton.style.display = 'inline-block';
        followButton.addEventListener('click', async () => {
          const response = await fetch(`/user/${id}/request-follow`, { method: 'POST' });
          const result = await response.json();
          if (result.success) {
            followButton.textContent = "Request Pending";
            followButton.disabled = true;
