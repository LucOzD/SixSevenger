...
-- ---------------------------------------------------------------- ads
-- Admin-authored sponsored cards are matched to a user's weighted category
-- centroids. Deliveries reserve frequency-cap slots; impressions/clicks are
-- filled idempotently by the frontend when the card is actually seen/used.
CREATE TABLE IF NOT EXISTS ads (
  id TEXT PRIMARY KEY, title TEXT NOT NULL, body TEXT NOT NULL, emoji TEXT,
  image_path TEXT, cta_url TEXT NOT NULL, cta_label TEXT NOT NULL,
  keywords TEXT NOT NULL, ad_vector TEXT NOT NULL,
  active INTEGER NOT NULL DEFAULT 0, starts_at INTEGER, ends_at INTEGER,
...
CREATE TABLE IF NOT EXISTS ad_deliveries (
  id TEXT PRIMARY KEY, ad_id TEXT NOT NULL, user_id TEXT NOT NULL,
  selected_at INTEGER NOT NULL, impression_at INTEGER, clicked_at INTEGER,
  FOREIGN KEY (ad_id) REFERENCES ads(id),
  FOREIGN KEY (user_id) REFERENCES users(id)
);
CREATE INDEX IF NOT EXISTS idx_ad_deliveries_user_ad
  ON ad_deliveries(user_id, ad_id, selected_at DESC);
CREATE INDEX IF NOT EXISTS idx_ad_deliveries_ad
  ON ad_deliveries(ad_id, selected_at DESC);